...
int XT_A_S_N_unb( FLA_Obj at, FLA_Obj b, FLA_Obj C, FLA_Obj tau )
{
  ...
  alphain = 1.0/(*( double * ) FLA_Obj_buffer_at_view(tau));
  ...
  while ( FLA_Obj_width( aLt ) < FLA_Obj_width( at ) ){
    ...
    *(double *) FLA_Obj_buffer_at_view(alpha1)=-alphain*(*ptr_temp);
    /*------------------------------------------------------------*/
